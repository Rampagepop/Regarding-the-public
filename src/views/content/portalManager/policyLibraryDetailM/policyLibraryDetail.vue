...
<!-- //"$t('card.policyLibraryPublish')" -->
<template>
  <div id="policy-library" style="width:100%">
    <yu-panel
      ref="panel"
      :title="$t('card.policyLibraryPublish')"
      class="adjust-height"
      clearable
      @search="fuzzyQuery"
      show-search-input
    ...
    >
      ...
      <template slot="right">
        <yu-toolBar>
          <!-- 上传   v-if="checkCtrl('upload')"-->
          <yu-button v-norepeat.disabled @click="uploadFile">
            新增
          </yu-button>
        </yu-toolBar>
      </template>
      ...
      >
        ...
        <yu-xtable-column fixed="right" label="操作">
          <template slot-scope="scope">
            <!-- v-if="checkCtrl('edit')" v-if="checkCtrl('delete')" -->
            <yu-button type="text" size="small" @click="editFile(scope.row)"
            >编辑</yu-button
            >
            <yu-button type="text" size="small" @click="deleteData(scope.row)"
            >删除</yu-button
            >
            <yu-button
        ...
        >
          <yu-xform-group :column="2">
            ...
            >
              ...
              ></yu-combo-tree>
              <!-- <yu-tooltip v-if="viewType === 'DETAIL'" placement="top" effect="light">
            ...
            <yu-xform-item
              ...
              name="fileIds"
            >
              <!-- :action="uploadAction"  :on-progress="fileProgress" :http-request="uploadFileHttp"-->
              <yu-upload
                v-if="viewType !== 'DETAIL'"
                class="upload-c"
                drag
                :action="$backend.portalService + '/api/xmhpolicy/upload'"
...
export default {
  ...
  data: function() {
    return {
      ...
      tableUrl: backend.portalService + "/api/xmhpolicy/searchEdit", // 用户列表查询地址
      ...
      uploadAction: yufp.util.addTokenInfo(
        backend.portalService + "file/provider/uploadfile"
      ), // 头像上传地址
  ...
  watch: {
    ...
    // fileResponseList:function(newVal, oldVal){
    //   handle
    // }
  },
  created() {
  ...
  methods: {
    ...
    saveData(type) {
      ...
      if(this.fileList.length === this.$refs.infForm.formdata.fileIds.length) {
        this.$request({
          ...
          if (res && res.code === "0") {
            ...
            if(this.removeData.fileIds.length && this.$refs.infForm.formdata.policyId) {
              this.$request({
      ...
      if(file.status === "success") {
        if (this.fileDtlInfos && this.fileDtlInfos.length) {
          this.fileDtlInfos.forEach((item, index) => {
            if (item.fileId === (file.response && file.response.data ? file.response.uid : file.uid)) {
        ...
        if(file && file.status === "success") {
          if (fileList.length) {
            fileList.forEach(item => {
              this.$refs.infForm.formdata.fileIds.push((item.response && item.response.data ? item.response.data : (item.uid + "|" + item.name)))
            });
          } else {
            this.$refs.infForm.formdata.fileIds = [];
          }  
    ...
    handleImageSuccess: function(res, file, fileList) {
      if(res && res.code === '0') {
        ...
        // file.uid = res.data.split('|')[0];//造成列表抖动
    ...
    exportFiled(event, type) {
      ...
      if (type === "applicableOrgs") {
        ...
        //做排序后对比数组是否相同
        if((allroles && allroles.length && allroles.sort().toString()) === choseroles.sort().toString()) {
    ...
    editFile(row) {
      ...
      this.$nextTick(() => {
        this.$refs.infForm.resetFields();
        this.resetData(); //先清空数据避免后期多次点击编辑展示上一条数据的值
        this.$store.dispatch("funData/rolesDataFn");
        this.getRoles();
    ...
    selectChangeSerch() {
      ...
      this.exportParam.applicableOrgs = [this.searchFormdata.applicableOrgs];
    },
    selectChange() {},
    inputBlur() {
    ...
    policyLibraryDetailFn: function(row) {
      ...
      _this.$nextTick(function() {
        ...
        _this.$refs.infForm.formdata = clone(row, _this.$refs.infForm.formdata);
        this.applicableOrgs = _this.$refs.infForm.formdata.applicableOrgs;
        this.applicableRoles = _this.$refs.infForm.formdata.applicableRoles;
...
<style>
/* 不要动态的动画 */
#policy-library .el-upload-list__item.is-ready,
#policy-library .el-upload-list__item.is-uploading {
