  ...
  <div id="beforeCreditInfo">
    <yu-panel>
      <yu-panel :title="$t('toolManager.dqss')">
        <yu-xform ref="refForm" label-width="450px" v-model="formdata" height="450px">
          <yu-xform-group>
            ...
            <yu-xform-item :label="$t('toolManager.hkfs')" ctype="select" name="planCode" data-code="REPAY_MODE" :rules="inputRules" @change="resetOthers"></yu-xform-item>
            <yu-xform-item :label="$t('toolManager.khr')" name="opnDate" ctype="datepicker" :rules="inputRules" :hidden="formdata.planCode=='07'" @change="compaStrtToEndDateOpn"></yu-xform-item>
            <yu-xform-item :label="$t('toolManager.dqr')" name="mtrDate" ctype="datepicker" :rules="inputRules" @change="compaStrtToEndDateOpn" :hidden="formdata.planCode=='07'"></yu-xform-item>
            ...
            <yu-xform-item :label="$t('toolManager.zdhkr')" ctype="num" name="repayDate" :hidden="formdata.planCode=='07'" :rules="inputRules"></yu-xform-item>
            <yu-xform-item :label="$t('toolManager.hkzq')" ctype="select" name="cycleTerm" data-code="CYCLE_TERM" :hidden="formdata.planCode=='07'" :rules="inputRules"></yu-xform-item>
            <yu-xform-item :label="$t('toolManager.hkzqjg')" ctype="input" name="cycleSpace" :rules="inputRules" :hidden="formdata.planCode=='07'"></yu-xform-item>
      ...
      <yu-panel :title="$t('toolManager.hkjhxx')" v-if="formdata.planCode=='07'">
        <yu-button-drop>
        ...
        <yu-xtable selection-type="radio" row-number ref="refTable1" :data="tableData1" :default-load="false" style="width: 100%" stripe @row-click="rowClickFn1">
          ...
          <yu-xtable-column :label="$t('toolManager.hkfs')" prop="repyMode" :disabled="disabled" ctype="select" data-code="ZU_REPAY_MODE" :rules="inputRules"></yu-xtable-column>
      ...
      <div class="button-group" style="text-align: center; margin-top:10px">
        <el-button type="primary" @click="submitButton">{{ $t('toolManager.sscc') }}</el-button>
    ...
    <yu-xdialog :title="$t('toolManager.hkfa')" :visible.sync="repayDialog" width="80%" top="5%" :middle="false" height="450px">
      <yu-xform ref="refForm2" label-width="120px" v-model="formdataPlanInfo">
        <yu-xform-group>
          <yu-xform-item :label="$t('toolManager.hkze')" :placeholder="$t('toolManager.hkze')" name="sumPrcplAmt" ctype="yu-num" :formatter="formFormatter" :precision="2" number-formatter="0.00" class="label-padding" readonly></yu-xform-item>
          <yu-xform-item :label="$t('toolManager.lxze')" :placeholder="$t('toolManager.lxze')" name="sumIntAmt" ctype="yu-num" :formatter="formFormatter" :precision="2" number-formatter="0.00" readonly></yu-xform-item>
        </yu-xform-group>
      </yu-xform>
      ...
      <div slot="footer" class="yu-grpButton">
        <yu-button type="primary" @click="returnFnRepay" plain>{{ $t('toolManager.fh') }}</yu-button>
...
lookup.reg("CUR_CODE,CYCLE_TERM,ZU_REPAY_MODE,REPAY_MODE,SUBSIDE_FLAG,SUBSIDY_TYPE,REPAY_PERIOD");
...
export default {
  ...
  methods: {
    ...
    formResetFn: function() {
      ...
      _this.list.splice(0, this.list.length);
    },
    ...
    submitButton: function() {
      ...
      if (this.formdata.planCode == "07") {
        ...
        if(!this.formdata.curCode || !this.formdata.cycleSpace || !this.formdata.cycleTerm ||
        !this.formdata.lnAmt || !this.formdata.loanRate || !this.formdata.mtrDate || !this.formdata.opnDate || !this.formdata.planCode
        || !this.formdata.repayDate || !this.formdata.subsidyFlag) {
          _this.$message({
            message: this.$t('toolManager.qsrwzxx'),
            type: "warning"
          });
...
::v-deep .yu-xform .el-form-item__content[aria-ctype="select"], .yu-xform .el-form-item__content[aria-ctype="input"], .yu-xform .el-form-item__content[aria-ctype="num"] {
