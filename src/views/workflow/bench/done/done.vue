...
<template>
  <div>
    <yu-panel :title="$t('wfdonelist.title')" :collapse-hide="false">
      <yu-xform v-model="formdata" related-table-name="reftable" ref="searchForm" label-width="120px">
        <yu-xform-group :column="4">
          ...
          <yu-xform-item :label="$t('wfdonelist.lastusername')" :placeholder="$t('wfdonelist.lastusername')" ctype="input" name="lastUserName"></yu-xform-item>
      ...
      <yu-xtable ref="reftable" :row-index="true" request-type="POST" :base-params="baseParams" :data-url="urls.index" show-hidden-menu>
        <yu-xtable-column :label="$t('wfdonelist.lcslh')" prop="instanceId" :resizable="true" min-width="120px" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <a class="underline" @click="rowDblclick(scope.row)">{{ scope.row.instanceId }}</a>
          </template>
        </yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.ywlsh')" prop="bizId" :resizable="true" min-width="120px" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.flowname')" prop="flowName" :resizable="true" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.khbh')" prop="bizUserId" :resizable="true" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.khmc')" prop="bizUserName" :resizable="true" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.nodename')" prop="nodeName" :resizable="true" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.lastnodename')" prop="lastNodeName" :resizable="true" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.lastusername')" prop="lastUserName" :resizable="true" :show-overflow-tooltip="true"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.starttime')" prop="startTime" :resizable="true" min-width="100px" :show-overflow-tooltip="true" :formatter="formatData"></yu-xtable-column>
        <yu-xtable-column :label="$t('wfdonelist.nodestate')" prop="nodeState" :resizable="true" align="center" min-width="85px" :show-overflow-tooltip="true">
          <template slot-scope="scope">
        ...
        <yu-xtable-column :label="$t('wfdonelist.flowstate')" prop="flowState" :resizable="true" align="center" min-width="85px" :show-overflow-tooltip="true">
          <template slot-scope="scope">
        ...
        <yu-xtable-column :label="$t('wfdonelist.biztype')" prop="bizType" :resizable="true" align="center" :hide-column="true"></yu-xtable-column>
      </yu-xtable>
    </yu-panel>
  </div>
</template>
...
export default {
  ...
  methods: {
    rowDblclick: function (row, event) {
      ...
      if (row.length < 1) {
        _this.$message({
          message: _this.$t('wfdonelist.qxxzytjl'),
          type: 'warning'
        });
      ...
      if (row.flowState == 'H') {
        _this.$message({
          message: _this.$t('wfdonelist.lcslcygqztwfjxcz'),
          type: 'warning'
        });
      } else {
