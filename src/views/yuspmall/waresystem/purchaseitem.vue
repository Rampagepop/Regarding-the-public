...
export default {
  ...
  methods: {
    mergeItem() {
      ...
      if (!this.purchaseId) {
        ...
        this.$request({
          url: "/api/ware/purchase/merge",
          method: "post",
          data: { purchaseId: this.purchaseId, items: items },
        }).then(() => {
          this.getDataList();
        });
      }
    ...
    getUnreceivedPurchase() {
      this.$request({
        url: "/api/ware/purchase/unreceive/list",
      }).then(({ data }) => {
    ...
    getDataList() {
      ...
      this.$request({
        url: "/api/ware/purchasedetail/list",
        data: {
          page: this.pageIndex,
          size: this.pageSize,
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定对[id=${ids.join(",")}]进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$request({
          url: "/api/ware/purchasedetail/delete",
          method: "post",
          data: ids,
        }).then(({ code, message }) => {
          if (code == "0") {
