            ...
            >
              ...
              <el-form ref="form" :model="spu">
                <el-form-item :label="attr.attrName" v-for="(attr, aidx) in group.attrs" :key="attr.attrId">
                  ...
                  <el-checkbox v-model="dataResp.baseAttrs[gidx][aidx].showDesc" :true-label="1" :false-label="0"
                    >快速展示</el-checkbox
                  >
                </el-form-item>
              </el-form>
                  ...
                  <el-row>
                    <el-col :span="24">
                      <el-form-item label="设置折扣">
                        ...
                        ></el-input-number>
                        <label>折</label>
                        <el-checkbox v-model="scope.row.countStatus" :true-label="1" :false-label="0"
                          >可叠加优惠</el-checkbox
                        >
                      </el-form-item>
                    </el-col>
                    <el-col :span="24">
                      <el-form-item label="设置满减">
                        ...
                        ></el-input-number>
                        <label>元</label>
                        <el-checkbox v-model="scope.row.priceStatus" :true-label="1" :false-label="0"
                          >可叠加优惠</el-checkbox
                        >
                      </el-form-item>
                    </el-col>
  ...
  methods: {
    ...
    getMemberLevels() {
      this.$request({
        ...
        .then(({ data }) => {
          this.dataResp.memberLevels = data;
        })
        .catch((e) => {
          console.log(e);
        });
    ...
    getShowSaleAttr() {
      ...
      if (!this.dataResp.steped[1]) {
        this.$request({
          url: `/api/product/attr/sale/list/${this.spu.catalogId}`,
          data: { page: 1, size: 500 },
        }).then(({ code, data }) => {
          if (code == "0") {
            this.dataResp.saleAttrs = data;
            data.forEach((item) => {
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$request({
          url: `/api/product/attrgroup/${this.spu.catalogId}/withattr`,
        }).then(({ code, data }) => {
          if (code == "0") {
            //先对表单的baseAttrs进行初始化
            data.forEach((item) => {
