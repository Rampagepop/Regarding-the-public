...
export default {
  ...
  methods: {
    ...
    attrUpdateShow(row) {
      ...
      const route = "yuspmall/productsystem/attrupdate";
      const title = "商品规格维护";
      const path = "/" + "attrupdate";
      this.$router.addRoute(route, title, {}, path); // 第三个参数meta详情见VUE官方的router参数
      this.$router.push({ path: path, query: { spuId: row.id, catalogId: row.catalogId } }); // query 可传递新页面初始化加载的参数
      // let routeName = "attrupdate";
      // let route = {
      //   path: routeName,
      //   component: () => import("@/views/yuspmall/productsystem/attrupdate"),
      //   name: routeName,
      //   meta: {
      //     menuId: 100,
      //     title: "规格维护",
      ...
      // this.$router.options.routes[2].children.push(route)
      // this.$router.addRoutes(this.$router.options.routes)// 调用addRoutes添加路由
      // 可以通过在菜单配置中增加一个菜单项-规格维护 url: product/atttrupdate
      // this.$router.push({
      //   path: "product-attrupdate",
      //   name: "product-attrupdate",
